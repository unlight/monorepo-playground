...
const pluginContextSymbol = Symbol('PluginContext');
const getPluginContext = context => context[pluginContextSymbol];
...
exports.verifyConditions = async function verifyConditions(pluginConfig, context) {
  const errors = [];
  const pluginContext = (context[pluginContextSymbol] = {});
  const { cwd } = context;
